...
def main():
    ...
    # SONG 집계 처리
    song_counts = df.groupBy("SONG", "ARTIST")\
                    .count()\
    ...
    # Snowflake에 저장
    song_counts.write.format("snowflake") \
        .options(**snowflake_gold_options) \
        .option("dbtable", SNOWFLAKE_TARGET_SONG_TABLE) \
        .mode("overwrite") \
        .save()

    artist_counts.write.format("snowflake") \
        .options(**snowflake_gold_options) \
        .option("dbtable", SNOWFLAKE_TARGET_ARTIST_TABLE) \
        .mode("overwrite") \
        .save()
