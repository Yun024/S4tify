...
def search_artist_id(artist_name):
    ...
    if response.status_code == 200:
        ...
        print(f"❌ 아티스트 검색 실패: {artist_name}, 상태 코드: {response.status_code}, 응답: {response.json()}")
    return None
...
def get_artist_genre(artist_id):
    ...
    if response.status_code == 200:
        ...
        print(f"❌ 장르 검색 실패: ID {artist_id}, 상태 코드: {response.status_code}, 응답: {response.json()}")
    return "Unknown"
...
def convert_json_to_csv(**kwargs):
    ...
    for entry in data["entries"]:
        csv_data.append([
            entry["rank"], entry["title"], entry["artist"], entry["lastPos"], entry["peakPos"], entry["image"], entry["genre"]
        ])
